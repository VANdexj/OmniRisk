...
struct StaticPole { float cx, cy, r, height; };
struct StaticWall { float cx, cy, half_len_x, half_thick_y, height; };
...
public:
  ...
  const std::vector<StaticPole>& getStaticPoles() const { return static_poles_; }
  const std::vector<StaticWall>& getStaticWalls() const { return static_walls_; }
  float getArenaXHalf() const { return arena_x_half_; }
  float getArenaYHalf() const { return arena_y_half_; }
...
private:
  ...
  // dynamic arena
  int    pole_num;
  double pole_radius_min, pole_radius_max;
  int    arena_ceiling;
  int    arena_walls;
  std::vector<StaticPole> static_poles_;
