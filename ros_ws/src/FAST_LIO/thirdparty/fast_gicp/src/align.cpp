...
// cxd
//  std::vector<Eigen::Isometry3d, Eigen::aligned_allocator<Eigen::Isometry3d>> poses; // stores poses 
Eigen::Matrix4f poses;

//gicp.getFinalTransformation().cast<double>() // returns a Lie group element
...
void test(Registration& reg, const pcl::PointCloud<pcl::PointXYZ>::ConstPtr& target, const pcl::PointCloud<pcl::PointXYZ>::ConstPtr& source) {
  ...
  double reuse = std::chrono::duration_cast<std::chrono::nanoseconds>(t2 - t1).count() / 1e6;
   // matrix storing the pose transform
...
int main(int argc, char** argv) {
  ...
  // figure out which one is source and which is target
