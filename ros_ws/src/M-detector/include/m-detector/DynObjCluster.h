...
public:
    ...
    float near_range_cluster_trustable_thresold = 0.3f;
    int time_ind = 0;
    ...
    Eigen::Vector3f configured_xyz_origin = Eigen::Vector3f(-100.0f, -100.0f, -20.0f);
    Eigen::Vector3f configured_maprange = Eigen::Vector3f(200.0f, 200.0f, 40.0f);
    ...
    void Clusterprocess(std::vector<int> &dyn_tag,
                        ...
                        const std::vector<uint8_t> *near_range_fast_tag = nullptr);
    ...
    void PubClusterResult_voxel(std::vector<int> &dyn_tag, std_msgs::Header current_header, bbox_t &bbox, double delta,
                                std::vector<std::vector<int>> &voxel_clusters, const pcl::PointCloud<PointType> &raw_point, std::unordered_set<int> &used_map_set, const std::vector<uint8_t> *near_range_fast_tag = nullptr);
