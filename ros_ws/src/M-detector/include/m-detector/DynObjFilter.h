...
struct DetectorFrame {
    ...
    std::optional<std::vector<m_detector::Detection3D>> preclustered_detections;
    bool frame_out_available = false;
};

// Invoked synchronously at most once when compact detections are ready, before
// the full output clouds are rebuilt. A later full-output failure does not roll
// back this delivery, so callers must treat a completed callback as processed
// even when filter() subsequently returns std::nullopt.
using FastDetectorFrameCallback = std::function<void(const DetectorFrame &)>;
...
public:
    ...
    bool publish_point_out_en = true;
    bool publish_frame_out_en = true;
    bool publish_steady_en = true;
    ...
    std::vector<uint8_t> near_range_fast_tag;
    ...
    float near_range_fast_confirm_distance = 1.0f;
    int near_range_case1_occluded_map_thr = 2;
    int near_range_case3_occluded_times_thr = 2;
    ...
    int raw_case1_dyn_num = 0, raw_case2_dyn_num = 0, raw_case3_dyn_num = 0;
    int final_case1_dyn_num = 0, final_case2_dyn_num = 0, final_case3_dyn_num = 0;
    int promoted_static_dyn_num = 0;
    ...
    bool case1_trace_all_en = false;
    std::vector<uint8_t> last_raw_dyn_labels;
    std::vector<std::string> last_case1_replay_traces;
    ...
    void shutdown();
    std::optional<DetectorFrame> filter(PointCloudXYZI::Ptr feats_undistort,
                                        const M3D &rot_end,
                                        const V3D &pos_end,
    ...
    void publish_dyn(const ros::Publisher &pub_point_out,
                     ...
                     const double &scan_end_time,
                     // Output clouds (point_out/frame_out/steady_points) use this exact stamp in their header
                     // instead of a ros::Time().fromSec(scan_end_time) double round trip, which at current
                     // epoch magnitudes loses tens to hundreds of nanoseconds, so downstream exact nanosecond
                     // matching (planner-side select_latest_complete) would never succeed. scan_end_time
                     // itself is still used for internal time differences.
                     const ros::Time &scan_stamp = ros::Time(0));
    void set_path(string file_path, string file_path_origin);
    const std::vector<uint8_t> &lastRawDynLabels() const { return last_raw_dyn_labels; }
    const std::vector<int> &lastClusterDynTags() const { return dyn_tag_cluster; }
    const std::vector<std::string> &lastCase1ReplayTraces() const { return last_case1_replay_traces; }
    std::string DebugCase1ReplayForRawPoint(const PointType &point,
                                            const M3D &rot_end,
                                            const V3D &pos_end,
                                            const double &scan_end_time);
    ...
    bool IsNearRangeFastPoint(const point_soph &p) const;
    int Case1OccludedMapThreshold(const point_soph &p) const;
    int Case3OccludingTimesThreshold(const point_soph &p) const;
    bool Case1(point_soph &p);
    std::string TraceCase1Replay(point_soph p);
    ...
    float DepthInterpolationStaticNew(point_soph &p, int map_index, const DepthMap2D &depth_map);
