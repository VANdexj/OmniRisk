...
void runCpuVoxelCluster(pcl::PointCloud<PointType>::Ptr points_in,
                        std::vector<Point_Cloud> &umap_in,
                        std::unordered_set<int> &used_map_set,
                        std::vector<std::vector<int>> &voxel_clusters,
                        float voxel_resolution,
                        int grid_edge_size_xy,
                        int grid_edge_size_z,
                        const Eigen::Vector3f &xyz_origin,
                        int cluster_extend_pixel,
                        int cluster_min_pixel_number) {
    VOXEL_CLUSTER cluster;
    cluster.setInputCloud(*points_in);
    cluster.setVoxelResolution(voxel_resolution, grid_edge_size_xy, grid_edge_size_z, xyz_origin);
...
void DynObjCluster::Init() {
    ...
    xyz_origin = configured_xyz_origin;
    maprange = configured_maprange;
    if (Voxel_revolusion <= 0.0f) {
        ROS_ERROR_STREAM("Invalid cluster_Voxel_revolusion=" << Voxel_revolusion
                         << ", reset to 0.3");
        Voxel_revolusion = 0.3f;
    }
    if (maprange.x() <= 0.0f || maprange.y() <= 0.0f || maprange.z() <= 0.0f) {
        ROS_ERROR_STREAM("Invalid cluster map range=(" << maprange.x() << ", "
                         << maprange.y() << ", " << maprange.z()
                         << "), reset to default 200 x 200 x 40");
        maprange << 200.0f, 200.0f, 40.0f;
    }
    ...
    ROS_INFO_STREAM("DynObjCluster map configured: origin=(" << xyz_origin.x() << ", "
                    << xyz_origin.y() << ", " << xyz_origin.z()
                    << "), range=(" << maprange.x() << ", " << maprange.y()
                    << ", " << maprange.z() << "), voxel=" << Voxel_revolusion
                    << ", grid_edges=(" << GridMapedgesize_xy << ", "
                    << GridMapedgesize_xy << ", " << GridMapedgesize_z
                    << "), grid_size=" << GridMapsize);
...
void DynObjCluster::shutdown() {
}


void DynObjCluster::Clusterprocess(std::vector<int> &dyn_tag, const pcl::PointCloud<PointType> &event_point, const pcl::PointCloud<PointType> &raw_point, const std_msgs::Header &header_in, const Eigen::Matrix3d odom_rot_in, const Eigen::Vector3d odom_pos_in, const std::vector<uint8_t> *near_range_fast_tag) {
    ...
    pcl::PointCloud<PointType>::Ptr cloud_clean_ptr(new pcl::PointCloud<PointType>(event_point));
    bbox_t bbox_high;
    ClusterAndTrack(dyn_tag, cloud_clean_ptr, pub_pcl_before_high, header, pub_pcl_after_high, cluster_vis_high, predict_path_high, bbox_high, delta_t, raw_point, near_range_fast_tag);
...
void DynObjCluster::ClusterAndTrack(
    ...
    const std::vector<uint8_t> *near_range_fast_tag) {
    std::vector<pcl::PointIndices> cluster_indices;
    std::vector<std::vector<int>> voxel_clusters;
    ros::Time t0 = ros::Time::now();
    std::unordered_set<int> used_map_set;
    GetClusterResult_voxel(points_in, umap, voxel_clusters, used_map_set);
    PubClusterResult_voxel(dyn_tag, header_in, bbox, delta, voxel_clusters, raw_point, used_map_set, near_range_fast_tag);
}
...
void DynObjCluster::GetClusterResult_voxel(pcl::PointCloud<PointType>::Ptr points_in, std::vector<Point_Cloud> &umap_in, std::vector<std::vector<int>> &voxel_clusters, std::unordered_set<int> &used_map_set) {
    ScopedTiming cluster_timer("cpu_voxel_cluster");
    ros::Time t0 = ros::Time::now();
    if ((out_file != "") && points_in->size() < 2) {
        out << (ros::Time::now() - t0).toSec() << " ";
        return;
    }
    runCpuVoxelCluster(points_in, umap_in, used_map_set, voxel_clusters,
                       Voxel_revolusion, GridMapedgesize_xy, GridMapedgesize_z,
                       xyz_origin, cluster_extend_pixel, cluster_min_pixel_number);
...
void DynObjCluster::PubClusterResult_voxel(std::vector<int> &dyn_tag, std_msgs::Header current_header, bbox_t &bbox, double delta,
                                           std::vector<std::vector<int>> &voxel_clusters, const pcl::PointCloud<PointType> &raw_point, std::unordered_set<int> &used_map_set, const std::vector<uint8_t> *near_range_fast_tag) {
    int j = 0;
    Eigen::Matrix3f R = odom_rot.cast<float>();
    Eigen::Vector3f world_z = R.col(2);
    const float inv_voxel_resolution = 1.0f / Voxel_revolusion;

    ScopedTiming bbox_build_timer("cpu_cluster_bbox_build");
    bbox.Point_cloud.reserve(voxel_clusters.size());
    bbox.Point_indices.reserve(voxel_clusters.size());
    bbox.Center.reserve(voxel_clusters.size());
    bbox.Ground_points.reserve(voxel_clusters.size());
    bbox.true_ground.reserve(voxel_clusters.size());
    bbox.Ground_voxels_set.reserve(voxel_clusters.size());
    bbox.Ground_voxels_vec.reserve(voxel_clusters.size());
    bbox.umap_points_num.reserve(voxel_clusters.size());
    ...
    for (auto it = voxel_clusters.begin(); it != voxel_clusters.end(); it++, j++) {
        ...
        const bool shape_ok = m_detector::ClusterShapeAccepted(
            cluster_min_pixel_number,
            Voxel_revolusion,
            x_size,
            y_size,
            z_size);
        if (shape_ok) {
            ...
            bbox.Point_cloud.back().reserve(n);
            bbox.Point_indices.emplace_back();
            bbox.Point_indices.back().reserve(n);
    ...
    bbox_build_timer.stop();

    std::vector<int> index_bbox(bbox.Center.size());
    for (int i = 0; i < bbox.Center.size(); i++) {
        index_bbox[i] = i;
    }
    ScopedTiming bbox_expand_timer("cpu_cluster_bbox_build");
    ...
    std::for_each(std::execution::par, index_bbox.begin(), index_bbox.end(), [&](const int &bbox_i) {
        ...
        int n_x = static_cast<int>(std::max(1.0f, x_size) * inv_voxel_resolution);
        int n_y = static_cast<int>(std::max(1.0f, y_size) * inv_voxel_resolution);
        int n_z = static_cast<int>(std::max(1.0f, z_size) * inv_voxel_resolution);
        int voxel_center = static_cast<int>(std::floor((center.x - xyz_origin(0)) * inv_voxel_resolution)) * GridMapedgesize_xy * GridMapedgesize_z
                         + static_cast<int>(std::floor((center.y - xyz_origin(1)) * inv_voxel_resolution)) * GridMapedgesize_z
                         + static_cast<int>(std::floor((center.z - xyz_origin(2)) * inv_voxel_resolution));
        int ii = 0;
        Eigen::Vector3f xyz;
    ...
    bbox_expand_timer.stop();

    ScopedTiming point_assign_timer("cpu_cluster_point_assign");
    for (int ite = 0; ite < raw_point.size(); ite++) {
        if (dyn_tag[ite] == -1)
            continue;
        int voxel = static_cast<int>(std::floor((raw_point[ite].x - xyz_origin(0)) * inv_voxel_resolution)) * GridMapedgesize_xy * GridMapedgesize_z
                  + static_cast<int>(std::floor((raw_point[ite].y - xyz_origin(1)) * inv_voxel_resolution)) * GridMapedgesize_z
                  + static_cast<int>(std::floor((raw_point[ite].z - xyz_origin(2)) * inv_voxel_resolution));
    ...
    std::for_each(std::execution::par, index_bbox.begin(), index_bbox.end(), [&](const int &k) {   
        ...
        bool has_near_range_fast_point = false;
        if (near_range_fast_confirm_en && near_range_fast_tag != nullptr)
        {
            for (int point_index : bbox.Point_indices[k])
            {
                if (point_index >= 0 &&
                    point_index < static_cast<int>(near_range_fast_tag->size()) &&
                    (*near_range_fast_tag)[point_index] != 0)
                {
                    has_near_range_fast_point = true;
                    break;
                }
            }
        }
        if (!m_detector::ClusterEvidenceAccepted(
                bbox.umap_points_num[k],
                static_cast<int>(bbox.Point_cloud[k].size()),
                has_near_range_fast_point,
                thrustable_thresold,
                near_range_cluster_trustable_thresold)) // not trustable
        {   
    ...
    {
        ScopedTiming timer("cpu_cluster_cleanup");
